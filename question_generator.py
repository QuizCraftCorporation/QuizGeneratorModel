...
class TutorialGeneratorModel():

    def __init__(self) -> None:
        
        self.create_tokenizer = AutoTokenizer.from_pretrained("./question_model/")
        self.create_model = AutoModelForSeq2SeqLM.from_pretrained("./question_model/")
        self.distract_tokenizer = AutoTokenizer.from_pretrained("./distraction_model/")
        self.distract_model = AutoModelForSeq2SeqLM.from_pretrained("./distraction_model/")
        
    def create_tutorial(self, text_chunks: list[str]):
        result = []
        logging.info(f"Start processing text chunks. Number of chunks is {len(text_chunks)}.")
        for i in range(len(text_chunks)):
            text_chunks[i] = text_chunks[i].replace('\n', '')
            inputs = self.create_tokenizer(text_chunks[i], return_tensors="pt")
            ...
            logging.info(f"Question created - {i+1}")
            distractions = self._get_distractions(text_chunks[i], question, answer)
            logging.info(f"Distractions created - {i+1}")
            result.append([question, distractions + [answer]])
            logging.info(f"Processed {i+1}/{len(text_chunks)} chunks of text.")
        return result
